...
const expectedBuild = "20260905-story-photo-submit-r244";
...
if (!bundledSharedLifecycleUtils.includes("function snapshotSignature(snapshot)")
    ...
    || !bundledMobileJs.includes("ANDROID_LIFECYCLE_SESSION_KEY")
    || !bundledMobileJs.includes("sessionStorage[ANDROID_LIFECYCLE_SESSION_KEY] ||=")
    || !bundledMobileJs.includes("snapshot.sessionId !== ANDROID_LIFECYCLE_SESSION_ID")
...
for (const lifecycleRuntime of [bundledApp, bundledLiveRuntime]) {
  if (!lifecycleRuntime.includes("if (state.mobileStartupRendering)")
      ...
      || !lifecycleRuntime.includes("ANDROID_LIFECYCLE_SESSION_KEY")
      || !lifecycleRuntime.includes("sessionStorage[ANDROID_LIFECYCLE_SESSION_KEY] ||=")
      || !lifecycleRuntime.includes("snapshot.sessionId !== ANDROID_LIFECYCLE_SESSION_ID")

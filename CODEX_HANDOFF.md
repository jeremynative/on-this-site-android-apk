...
- Signed release `0.1.630` passed the notification-tap return test on the iPlay 50 mini Pro. Its `Exit navigation` action stopped guidance and closed the route activity, but physical QA found that Android retained the stale foreground notification after the service ended. Follow-up branch `fix/navigation-notification-dismiss-20260901` explicitly cancels that notification after stopping guidance and orders the no-activity fallback cleanup after the Navigator callback.
- Follow-up build marker is `20260901-navigation-notification-dismiss-r218`. The Galaxy S25 is not currently attached.

Safest next action: run the focused checks and optimized build, publish the follow-up signed APK, install it over production without clearing data, and confirm `Exit navigation` removes the notification as well as stopping guidance.
...
Safest next action: merge and publish the edge-indicator follow-up, verify the signed artifact uses the production key and certificate, install it over production without clearing data, rerun the live route/edge/Add stop checks, then remove the QA app and local temporary key material.
...
## 2026-09-01 quota-safe custom-place search

- Android PR #72 merged to `main` as `3f02dd00629a4cf81149225413184a9c4ade770d`. Native build marker is `20260901-map-search-quota-fallback-r221`.
- Search typing now uses only the local On This Site index. Mapbox Search Box suggestion/retrieval sessions and session tokens were removed. The visitor's explicit **Search the map** action sends one bounded Temporary Geocoding request with `autocomplete=false`.
- If that request cannot run because the free allowance or network is unavailable, the entered destination remains visible and the results panel offers **Open directions in Maps**.
- `verify-app-shell.js`, `verify-google-play-readiness.js`, `git diff --check`, the debug APK build, `lintDebug`, and `assembleOptimizedQa` passed. The QA package was installed on the connected iPlay 50 mini Pro tablet; typing a custom address showed the single explicit map-search action, and tapping it produced the result plus nearby On This Site places.
- Signed Obtainium release `0.1.634` (`apk-634`) is latest. Release workflow `33515171177` passed, the published APK reports package `com.nativelongisland.onthissite`, version code `634`, version name `0.1.634`, certificate SHA-256 `b17b4d4505513c52adfaa215cac95ee062431d22410938fa5c13d34a70025787`, and file SHA-256 `0EB9F653BE58C498D4E0260AD6339A4ADD7265B02262D9359FF1BF47E506FABD`.
- Google Play was not submitted. Safest next action: allow Obtainium to update the production app normally; no manual reinstall is required unless immediate device confirmation is wanted.

## 2026-09-01 tablet biography loading and site-open map stability

- Branch `fix/tablet-bio-load-stability-20260901` is based on Android `ce34ce7`; native build marker is `20260901-tablet-bio-detail-cache-r223`.
- The APK now bundles the generated detail files for all 439 listings and 118 knowledgebase articles. Biography opens use the local hashed detail manifest first and retain Directus only as a fallback, removing the article-body network wait that made biography icons feel extremely slow.
- Tablet detail opens keep the full header/map rectangle stable, and `NativeMapController.updateViewport` restores the captured camera immediately after padding changes before the next rendered frame. Phone detail mode retains the requested compact title-only header.
- Physical QA on iPlay tablet `T811MA256GB23516041180` passed for Sunksqua Weany, Elizabeth Thunder Bird Haile, Sachem Tackapousha, and Wyandanch. Their shells appeared in 89–96 ms and full articles in 466–848 ms from local `wiki-details` files, with no `wiki_articles` detail request. Map rectangle movement was 0 px for every tested site and biography (previous baseline: 127.8–141.5 px).
...
- Safest next action: merge the matching web change first, then merge this branch, wait for the signed Obtainium release, install that exact signed APK over production without clearing data, and repeat the focused tablet audit. Google Play remains out of scope unless separately requested.

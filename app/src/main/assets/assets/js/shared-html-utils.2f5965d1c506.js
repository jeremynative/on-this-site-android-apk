(function () {
  ...
  function cleanHtml(value, options = {}) {
    ...
    // Imported centered labels are block headings, not inline bold prose.
    template.content.querySelectorAll("center").forEach(element => {
      const block = document.createElement(element.textContent.length < 160 && !element.querySelector("img, iframe") ? "h4" : "div");
      block.append(...element.childNodes);
      element.replaceWith(block);
    });
    if (options.mode === "desktop") cleanDesktopTemplate(template, options);
    else cleanMobileTemplate(template, options);
    structureArticleContent(template.content);
    return template.innerHTML;
  }

  function structureArticleContent(root) {
    root.querySelectorAll("table:not([class])").forEach(table => {
      let frame = table.parentElement;
      if (!frame?.classList.contains("article-table-scroll")) {
        frame = document.createElement("div");
        frame.className = "article-table-scroll";
        table.replaceWith(frame);
        frame.append(table);
      }
      frame.tabIndex = 0;
      frame.setAttribute("role", "region");
      frame.setAttribute("aria-label", "Article table; scroll horizontally for more columns");
    });
    // Isolate legacy inline photographs before paragraph reflow. An image in a
    // long paragraph otherwise prevents reflow and leaves nearby prose beside it.
    for (let img; (img = [...root.querySelectorAll("img")].find(node => !node.closest("figure")));) {
      let media = img;
      while (media.parentElement && /^(A|SPAN)$/.test(media.parentElement.tagName) &&
        !media.parentElement.textContent.trim() && media.parentElement.querySelectorAll("img").length === 1) media = media.parentElement;
      const figure = document.createElement("figure");
      const paragraph = media.closest("p");
      if (paragraph) {
        const before = document.createRange(), after = document.createRange();
        before.selectNodeContents(paragraph); before.setEndBefore(media);
        after.selectNodeContents(paragraph); after.setStartAfter(media);
        const lead = paragraph.cloneNode(false), tail = paragraph.cloneNode(false);
        lead.append(before.cloneContents()); tail.append(after.cloneContents());
        figure.append(media);
        const meaningful = node => node.textContent.trim() || node.querySelector("img,iframe,br");
        paragraph.replaceWith(...[lead, figure, tail].filter(node => node === figure || meaningful(node)));
      } else {
        media.replaceWith(figure);
        figure.append(media);
      }
    }
    // Older imports put both the image and its credit inside figcaption.
    root.querySelectorAll("figure figcaption").forEach(caption => {
      const figure = caption.closest("figure");
      caption.querySelectorAll("img").forEach(img => {
        let media = img;
        while (media.parentElement !== caption && !media.parentElement.textContent.trim()) media = media.parentElement;
        figure.insertBefore(media, caption);
      });
    });
    const blocks = /^(?:P|DIV|FIGURE|FIGCAPTION|H[1-6]|BLOCKQUOTE|UL|OL|LI|TABLE|HR|DETAILS|IFRAME)$/;
    // Preserve blank-line paragraph boundaries and separate loose dated entries.
    [root, ...root.querySelectorAll("blockquote, div:not([class])")].forEach(container => {
      let run = [];
      const flush = () => {
      ...
      for (const node of [...container.childNodes]) {
        if (node.nodeType === 1 && blocks.test(node.tagName)) { flush(); continue; }
        if (node.nodeType === 1 && /^(STRONG|B|EM)$/.test(node.tagName) &&
          /^(?:\d{3,4}(?:\s*[-–]\s*\d{2,4})?$|\d+[.)]\s|(?:January|February|March|April|May|June|July|August|September|October|November|December)\s+\d)/.test(node.textContent.trim())) flush();
        if (node.nodeType === 3 && /\n\s*\n/.test(node.textContent)) {
          const parts = node.textContent.split(/\n\s*\n/);
          parts.forEach((part, index) => {
            const text = document.createTextNode(part);
            container.insertBefore(text, node);
            run.push(text);
    ...
    // A standalone bold label is a section heading, including legacy date labels.
    // Inline emphasis and complete bold sentences retain their paragraph role.
    root.querySelectorAll("p").forEach(paragraph => {
      const text = paragraph.textContent.trim(), label = paragraph.firstElementChild;
      if (!text || text.length >= 160 || paragraph.children.length !== 1 ||
        !label || !/^(STRONG|B)$/.test(label.tagName) || label.textContent.trim() !== text || /[.!?]$/.test(text)) return;
      const heading = document.createElement("h4");
      heading.append(...label.childNodes);
      paragraph.replaceWith(heading);
    });
    root.querySelectorAll("p, figcaption, h1, h2, h3, h4").forEach(block => {
      if (!block.textContent.trim() && !block.querySelector("img, iframe, br, a[href]")) block.remove();
    });
    // Reflow only unusually long prose, keeping every character and inline link.
    // Ranges preserve emphasis/footnotes; short author-written paragraphs stay intact.
    if (typeof Intl.Segmenter !== "function") return;
    const segmenter = new Intl.Segmenter("en", { granularity: "sentence" });
    root.querySelectorAll("p").forEach(paragraph => {
      const text = paragraph.textContent;
      if (text.length < 1100 || paragraph.querySelector("img, iframe, br, table, ul, ol")) return;
      const cuts = [0];
      for (const part of segmenter.segment(text)) {
        const end = part.index + part.segment.length;
        const prefix = text.slice(0, end);
        if (end - cuts.at(-1) >= 500 && text.length - end >= 180 &&
          !/\b(?:Mr|Mrs|Ms|Dr|Prof|St|Sgt|Det|Rev|Inc|[A-Z])\.\s*$/.test(part.segment) &&
          (prefix.match(/\(/g) || []).length === (prefix.match(/\)/g) || []).length &&
          !/^\s*\(/.test(text.slice(end))) cuts.push(end);
      }
      ...
      let length = 0;
      while (walker.nextNode()) { nodes.push({ node: walker.currentNode, start: length }); length += walker.currentNode.length; }
      const boundary = offset => {
        const item = nodes.find(item => offset <= item.start + item.node.length) || nodes.at(-1);
        return [item.node, offset - item.start];
      };
      const fragment = document.createDocumentFragment();
      for (let index = 1; index < cuts.length; index++) {
        const range = document.createRange();
        range.setStart(...boundary(cuts[index - 1]));
        range.setEnd(...boundary(cuts[index]));
  ...
  function convertImportedFootnotes(value, options = {}) {
    ...
    const source = String(value || "").split(/(\r?\n[ \t]*\r?\n)/)
      .map(part => /^\r?\n[ \t]*\r?\n$/.test(part) ? "\n\n" : normalize(part)).join("");
    return source.replace(/\[(\d+)\.\s*([^\]]{8,300})\]/g, (_, number, note) => {

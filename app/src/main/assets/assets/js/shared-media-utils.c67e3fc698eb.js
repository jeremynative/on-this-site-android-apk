(function () {
  ...
  function listingHeroThumbnail(site) {
    const raw = cleanImageUrl(site?.listing_image_thumb_url);
    if (!raw) return "";
    const dimensions = raw.match(/-(\d+)x(\d+)\.(?:jpe?g|png|webp)(?:[?#].*)?$/i);
    return !dimensions || (Number(dimensions[1]) >= 760 && Number(dimensions[2]) >= 210) ? raw : "";
  }
